...
public final class Texture {
     ...
     * directions.</li>
     * <li><b>Clamp:</b> The texture's edge pixels are repeated beyond the
     * texture boundaries.</li>
    ...
    /**
     * Loads an image from the specified file path and initializes the texture
     * with its pixel data.
     *
     * @param path The path of the image file to load.
     * @throws RuntimeException If the image path is invalid or the image cannot
     * be loaded.
     */
     ...
     * @throws IndexOutOfBoundsException If the specified coordinates are
    ...
    /**
     * Samples the texture at the specified UV coordinates and returns the
     * sampled color as a Vector3. This method implements all the sampling and
     * wrap modes, but it exhibits slower performance due to the implementation
     * of multiple sampling and wrap modes. If performance is a concern and wrap
     * modes aren't a requirement, then using
     * {@link Texture#SampleFast(float, float)} is advised.
     *
    ...
    public Vector3 Sample(double u, double v) {
        ...
        switch (wrapMode) {
            case Clamp: {
                x = (int) MathHelper.clamp(x, 0, ((width - 1) * precision));
                y = (int) MathHelper.clamp(y, 0, ((height - 1) * precision));
                break;
            }
            case Repeat:
        ...
        if (samplingMode == SamplingMode.Point) {
            ...
            double xf = x / precision;
            double yf = y / precision;
            xf = MathHelper.clamp(xf, 0, width - 1);
            yf = MathHelper.clamp(yf, 0, height - 1);
            Vector2 tl = new Vector2((int) xf, (int) yf);
            Vector2 tr = new Vector2(Math.ceil(xf), (int) yf);
            Vector2 bl = new Vector2((int) xf, Math.ceil(yf));
            Vector2 br = new Vector2(Math.ceil(xf), Math.ceil(yf));
    ...
    /**
     * Fast sampling method that performs clamping and interpolation to sample
     * the texture at the specified UV coordinates. This method is faster than
     * the regular {@link Texture#Sample(float, float)} method, making it
     * advantageous when performance is a concern.
     *
     * @param u The U-coordinate of the texture.
     * @param v The V-coordinate of the texture.
     * @return The sampled color as a Vector3.
     */
    public Vector3 SampleFast(double u, double v) {
        double x = MathHelper.clamp(u * width, 0, width - 1);
        double y = MathHelper.clamp(v * height, 0, height - 1);

        if (samplingMode == SamplingMode.Point) {
            return GetPixelUv((int) x, (int) y);
        } else {
            Vector2 tl = new Vector2((int) x, (int) y);
            Vector2 tr = new Vector2(Math.ceil(x), (int) y);
            Vector2 bl = new Vector2((int) x, Math.ceil(y));
            Vector2 br = new Vector2(Math.ceil(x), Math.ceil(y));
